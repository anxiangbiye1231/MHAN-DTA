...
# %%
## from url=https://github.com/guaguabujianle/GIGN/tree/main/GIGN
...
def generate_complex(data_dir, data_df, save_dir, distance=5, input_ligand_format='mol2'):
    ...
    for i, row in data_df.iterrows():
        ...
        complex_dir = os.path.join(save_dir, cid)
...
if __name__ == '__main__':
    ...
    data_root = './dir' # data_root
    data_dir = os.path.join(data_root, 'test_hiq')
    data_df = pd.read_csv(os.path.join(data_root, 'test_hiq.csv'))

    ## generate pocket within 5 Ångström around ligand
    generate_pocket(data_dir=data_dir, distance=distance)
    # generate_complex(data_dir, data_df, data_root, distance=distance, input_ligand_format=input_ligand_format)
